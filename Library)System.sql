...
-- Advanced Queries
-- find books publlished after 1950
select * from books
where published_year >1949; 

-- Use joins to return american authors
select * from books as a
join authors as b on a.authorId = b.authorId
where b.nationality = 'American';

-- Set all books as true
update books set availability= true;

-- find available books 
 select * from books
 where availability= true and published_year >1949;

 -- use like to find names containing george
 select * from authors
 where a_name ilike '%George%';

 -- Incease year using update
 update books 
